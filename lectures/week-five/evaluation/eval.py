"""
eval.py — RAG evaluation metrics (retrieval + LLM-as-a-judge).

Part of Ed Donner's evaluation package (week5/evaluation). It scores:
  - Retrieval: MRR, nDCG, keyword coverage (using implementation.answer.fetch_context)
  - Answers:   accuracy / completeness / relevance via an LLM judge
                (using implementation.answer.answer_question)

Used by evaluator.py (the Gradio dashboard) and runnable as a CLI:
  uv run eval.py <test_row_number>

Adapted from ed-donner/llm_engineering (week5/evaluation/eval.py).
"""
