"""
evaluator.py — RAG Evaluation Dashboard (Ed Donner's version).

A Gradio dashboard that runs the evaluation package over the whole test set and
shows colour-coded retrieval metrics (MRR, nDCG, keyword coverage) and
LLM-as-a-judge answer scores (accuracy, completeness, relevance), with a bar
chart of the headline metric per question category.

This is the faithful course version. For my own extended take — tunable chunk
size/overlap/k, a pipeline selector (built-in / basic / pro), per-section
breakdowns and human-readable summaries — see
`homework-challenges/rag-evaluation-dashboard/evaluator.py`.

Run it:  python evaluator.py

Adapted from ed-donner/llm_engineering (week5/evaluator.py).
"""
