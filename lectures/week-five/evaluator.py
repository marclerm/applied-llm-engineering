"""
RAG Evaluation Dashboard (Week 5, Day 4).

A standalone Gradio app that evaluates the Insurellm RAG pipeline on two fronts:
...
It also lets you tune the pipeline live from the UI:
  - chunk size / chunk overlap (how documents are split → rebuilds the vector store)
  - retrieval k                (how many chunks are fetched per query → free to change)

This is the script version of `4-rag-evaluation.ipynb`. All of the evaluation
logic is inlined here so the file runs on its own:

    python lectures/week-five/evaluator.py

It reuses the persistent Chroma `vector_db` (built with OpenAI text-embedding-3-large
at the default chunk settings); other chunk settings are built into a temporary
in-memory store so your saved vector_db is never overwritten.
"""
