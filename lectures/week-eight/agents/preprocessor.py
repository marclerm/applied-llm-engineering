...
class Preprocessor:
    ...
    def preprocess(self, text: str) -> str:
        if self.model_name.startswith("groq/") and not os.getenv("GROQ_API_KEY"):
            raise RuntimeError(
                "GROQ_API_KEY is required because PRICER_PREPROCESSOR_MODEL uses Groq. "
                "Add it to the repository .env file, then restart the notebook kernel."
            )
